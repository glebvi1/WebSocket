...
async def run_3clients():
    ...
    4) Откл 3его, 2ого
    """
    clients = [asyncio.create_task(websocket_client()) for _ in range(3)]
    ...
    await asyncio.sleep(5)
    clients.append(asyncio.create_task(websocket_client()))
    print("Добавил")
...
async def run_30clients():
    clients = [asyncio.create_task(websocket_client()) for _ in range(30)]
